...
backup_wt=white
try:
    myFile=open('continue.txt','r')
except FileNotFoundError:
    newGame=True
    myFile=open('continue.txt','w')
    for (piece,coords) in black:
        temp=''
        for single in coords:
            temp+=single+' '
        myFile.write(temp + '\n')
    ...
    for abc in black:
        temporary=list(abc)
        linee=myFile.readline()
        temporary[1]=linee.split()
        templist.append(tuple(temporary))
    black=templist
    templist=[]
    for abc in white:
        temporary=list(abc)
...
def mistake(theInput,expected):
    humans='stupid'
    while humans =='stupid':
        if theInput in expected:
            humans = 'alright'
            return theInput
        else:
            print('invalid input, try again sorry')
            theInput=str(input())
...
altp=black
if not newGame:
    ask=mistake(input('Would you like to start a new game or continue old game [saved as continue.txt] (type: start or cont): '),['start','cont'])
    if ask == 'start':
        black=backup_bk
        white=backup_wt
option = 'none'
if colour_init:
    inp=mistake(str(input('Would you like to use the multi-coloured version? (y/n) ')),['y','n'])
    if inp=='y' :
        option = 'colour'
    else:
        option=question1()
...
while match_result =='ongoing':
    ...
    try:
        ...
        print('Please input TWO coordinates seperated by a space')
        old='nope'
        new='nada'
    for y in range(1,9):
        if new[0] ==str(y):
